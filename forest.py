...
class Forest(QDialog):  
    def __init__(self,parent=None):
        ...
        self.connect(self.listWidget,SIGNAL("currentRowChanged(int)"),self,SLOT("buttonStatus()"))  
...
class SetProcess(QWidget):
    ...
    def amend(self):
        ...
        self.forest.rel = forest.relevancyList[self.relevancyComboBox.currentText().encode('utf8')]
        
        self.label22.setText(self.normalizeComboBox.currentText())
        self.forest.normalize = forest.normalizeList[self.normalizeComboBox.currentText().encode('utf8')]
        
        self.label33.setText(self.similarityComboBox.currentText())
        self.forest.doSimilarity = forest.similarityList[self.similarityComboBox.currentText().encode('utf8')]
        
        self.label44.setText(self.predictModelComboBox.currentText())
        self.forest.predictModel = forest.predictModel[self.predictModelComboBox.currentText().encode('utf8')]
        return 'ok'
...
class SetDate(QWidget):
    ...
    def setDate(self,Forest):
        ...
        self.connect(self.cal, SIGNAL('selectionChanged()'), self.changePredictDate)
        self.cal.hide()
...
#            self.orderName.append(fieldDescription.get(value))
    ...
    def update(self):
        ...
        self.historyAtmTable = QTableWidget(len(self.forest.source)+len(self.forest.forest),len(order))
        self.historyAtmTable.setEditTriggers(QAbstractItemView.NoEditTriggers) 
        self.historyAtmTable.setHorizontalHeaderLabels([fieldDescription.get(value).decode('utf-8') for value in order])
        for i in range(len(self.forest.source)):
            for j in range(len(order)):
                newItem = QTableWidgetItem(str(self.forest.source[i][order[j]]))
                self.historyAtmTable.setItem(i,j,newItem)
        for i in range(len(self.forest.forest)):
            for j in range(len(order)):
                newItem = QTableWidgetItem(str(self.forest.forest[i][order[j]]))
                self.historyAtmTable.setItem(len(self.forest.source)+i,j,newItem)
        ...
        self.relTable.setHorizontalHeaderLabels([u'参数',u'相关度',u'是否采用',u'最小值'])
        i = 0
        for key,value in forest.relevancy.iteritems():
            newItem = QTableWidgetItem(fieldDescription.get(key).decode('utf-8'))
            self.relTable.setItem(i,0,newItem)
            newItem = QTableWidgetItem(str(value))
            self.relTable.setItem(i,1,newItem)
            newItem = QTableWidgetItem(abs(value)>=self.forest.minRel and u"是" or u"否")
    ...
    def amend(self):
        if self.done == False:
            ...
            for i in range(len(self.forest.similarity)):
                table = QTableWidget(len(self.forest.similarity[i]),len(order)+1)
                table.setEditTriggers(QAbstractItemView.NoEditTriggers)
                newOrder = [u'相似度']
                newOrder.extend([fieldDescription.get(value).decode('utf-8') for value in order])
                table.setHorizontalHeaderLabels(newOrder)
                for m in range(len(self.forest.similarity[i])):
                    newItem = QTableWidgetItem(str(self.forest.similarityValue[i][m]))
                    table.setItem(m,0,newItem)
                    for n in range(len(order)):
                        newItem = QTableWidgetItem(str(self.forest.similarity[i][m][order[n]]))
                        table.setItem(m,n+1,newItem)
                self.addTab(table,(self.forest.forest[i].date.isoformat()+'的相似日').decode('utf-8'))
            self.done = True
        return 'ok'
...
class DoPredict(QTabWidget):
    ...
    def amend(self):
        ...
        for i in range(len(self.forest.expect)):
            newItem = QTableWidgetItem(str(self.forest.forest[i].date.isoformat()))
